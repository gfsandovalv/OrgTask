...
class OrgBaseNode(Sequence):
    ...
    *** Heading 3
    ...
    def __init__(self, env, index=None) -> None:
        ...
        self._properties: Dict[str, PropertyValue] = {} # prop
        self._steps: Dict[str, PropertyValue] = {}
    ...
    @property
    def steps(self) -> Dict[str, PropertyValue]:
        """
        Node steps as a dictionary.
        ...
        ...   :STEPSEND:
        ... ''')
        >>> root.children[0].steps['SomeProperty']
        'value'

        """
        return self._steps

    def get_step(self, key, val=None) -> Optional[PropertyValue]:
        """
        Return property named ``key`` if exists or ``val`` otherwise.
        ...
        """
        return self._steps.get(key, val)
    ...
    def _iparse_steps(self, ilines: Iterator[str]) -> Iterator[str]:
        self._steps = {}
        in_property_field = False
        for line in ilines:
            if in_property_field:
                if line.find(":STEPSEND:") >= 0:
                    break
                else:
                    (key, val) = parse_property(line)
                    if key is not None and val is not None:
                        self._steps.update({key: val})
            elif line.find(":STEPS:") >= 0:
                in_property_field = True
            else:
                yield line
        for line in ilines:
...
class OrgRootNode(OrgBaseNode):
    ...
    def _parse_pre(self):
        ...
        ilines = self._iparse_steps(ilines)
